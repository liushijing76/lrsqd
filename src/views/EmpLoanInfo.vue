    ...
    <div>
        ...
        <el-form :inline="true" :model="select" class="demo-form-inline">
            <el-form-item label="借款金额">
                <el-input v-model="select.lbalance" clearable placeholder="请输入借款金额"></el-input>
            </el-form-item>
        ...
        >
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                    ...
                    <el-button
                            v-if="scope.row.lsname == '未还款' ||scope.row.lsname == '部分还款'"
                            size="mini"
                            type="primary"
                            @click="handleRepay(scope.$index, scope.row)">还款</el-button>
                    <el-button
                            v-if="scope.row.lsname == '员工确认'"
                            size="mini"
                            type="success"
                            plain
                    ...
                    <el-button
                            ...
                            size="mini"
                            type="success"
                            @click="submitEmpLoan(scope.$index, scope.row)">提交</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--
        ...
        <el-dialog width="30%" :title="title" :visible.sync="dialogFormVisible">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="saveEmpLoan">保 存</el-button>
            </div>
        </el-dialog>
        ...
        <el-dialog width="30%" :title="title" :visible.sync="dialogFormVisible03">
            <el-form ref="empLoanForm" :model="empLoanForm" :rules="rules03" >
                <el-form-item style="width: 400px" label="借款金额" :label-width="formLabelWidth03" prop="lbalance">
                    <el-input  v-model="empLoanForm.lbalance" autocomplete="off" readonly></el-input>
                </el-form-item>
                <el-form-item style="width: 400px" label="待还款金额" :label-width="formLabelWidth03" prop="stillrepay">
                    <el-input  v-model="empLoanForm.stillrepay" autocomplete="off" readonly></el-input>
                </el-form-item>
                <el-form-item style="width: 400px" label="请输入还款金额" :label-width="formLabelWidth03" prop="rbalance">
                    <el-input  v-model="empLoanForm.rbalance" autocomplete="off"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="dialogFormVisible03 = false">取 消</el-button>
                <el-button type="primary" @click="saveEmpRepay">保 存</el-button>
            </div>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            getLoanRecordList() {
                ...
                this.$http.post("/getLoanRecordList",this.select).then(res => {
            ...
            //提交员工借款信息
            submitEmpLoan(index,row) {
                this.$confirm('确认提交借款信息吗','提示',{
                    confirmButtonText:'确定',
                    cancelButtonText:'取消',
                    type:'warning'
                }).then(() => {
                    if(row.lbalance < 2000){
                        row.lsno = '3'
                    } else {
                        row.lsno = '2'
                    }
            ...
            delLoanInfo() {
                ...
                this.$confirm('确认要删除选中借款信息吗','提示',{
                    ...
                    for (let i = 0; i < selection.length ; i++) {
                        if(selection[i].lsno != 1){
                            this.$message.error( "已提交流程,删除失败");
                            return;
                        }
